...
| Japanese Text Analysis  | Recognized or displayed Japanese text                                       | The immutable SudachiDict Core dictionary ships in the app bundle and is read in place. Analysis remains on device; the app does not download its required default or copy it into Application Support.                                                                        | Uninstall                                                                                                                  | None                                                                             |
...
Version 1.0 has no account, cloud sync, analytics, advertising, third-party crash reporting SDK, or Zenbu-operated service. Its active app-operated download path is the learner-initiated, immutable-URL Frequency Pack flow described above. The generic Language Technology boundary retains checksum-validated support for future optional alternatives, but the required Sudachi Core default uses no network. Neither path sends learner content. On-device content is not data collected by Zenbu for App Store privacy-label purposes.
