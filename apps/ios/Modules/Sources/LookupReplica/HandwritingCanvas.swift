  ...
  var body: some View {
    GeometryReader { geometry in
      Canvas { context, size in
        ...
        context.stroke(grid, with: .color(.white.opacity(0.22)), style: StrokeStyle(lineWidth: 1, dash: [4, 4]))
      ...
      .accessibilityValue(strokes.isEmpty ? "Empty drawing" : "\(strokes.count) stroke\(strokes.count == 1 ? "" : "s")")
      .accessibilityIdentifier("handwriting.canvas")
    }
  }

  private func normalized(_ point: CGPoint, in size: CGSize) -> HandwritingPoint {
    HandwritingPoint(x: min(max(point.x / size.width, 0), 1), y: min(max(point.y / size.height, 0), 1))
  }
