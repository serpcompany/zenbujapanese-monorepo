...
The derived database is produced by `apps/ios/Tools/import_jmdict.py`. Zenbu retains JMdict `ent_seq` values only as source provenance and normalizes written forms, readings, individual ordered English gloss atoms, sense part-of-speech and written/reading applicability, reading restrictions, and complete form-scoped priority profiles into app-owned Language Reference Data. Dictionary Ranking consumes only that normalized lexical evidence and a provenance-free semantic fingerprint. Semantically equivalent rows retain every sorted unique source provenance while public identity deterministically uses the lexicographically smallest opaque app-owned ID; that identity normalization does not contribute Ranking fields. The import fails closed on unknown priority markers, invalid restrictions, count drift, semantic-fingerprint collisions, or source checksum drift, and records policy/schema versions, evidence counts, the deterministic evidence-mapping checksum, current importer/adapter checksums, and the artifact checksum. It also generates the bundled typed `DictionaryRankingArtifactContract.json`, which is the runtime's source of truth for those hashes and counts. `apps/ios/Tools/validate_dictionary_ranking_data.py` independently recomputes the evidence counts, three duplicate semantic groups/six source rows, and mapping, then validates the bundled database and contract against the pinned source, current tools, and manifest.

The current deterministic artifact is 390,746,112 bytes with SHA-256 `1d75bbc5983c396515f179eac5ccdbb5c849f5f1cf2f9c3835317af2a7849329`; its evidence mapping SHA-256 is `3b4b6bacff31b3e68ed9990af073ff8303405de82addbfbd8c652726052923fd`. Two independent rebuild copies were byte-identical. The generated runtime contract SHA-256 is `2d60eb12cdbbeae7ca5dab83ba1aefdf41dc0a509d6a8077e045075920c561e7`.
