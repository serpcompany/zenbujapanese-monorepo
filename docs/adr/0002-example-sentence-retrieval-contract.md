...
The shell selects this route only for a nonempty ASCII `SearchQuery`. The request uses the existing `SearchQuery` normalization: Unicode compatibility precomposition, lowercase, and collapsed whitespace. The adapter rejects an empty query or a query from which FTS4 Porter emits no term. It constructs one quoted phrase, doubles any embedded double quote, binds the complete MATCH expression as a parameter, and uses only static SQL identifiers. Quoting makes adjacency and order part of eligibility and prevents user text from becoming FTS operators. SQLite Porter—not Zenbu code—owns English token boundaries and stemming.
