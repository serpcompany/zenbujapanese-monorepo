...
const dispositionChecks = [
  ...
  { id: "AUDIT-CHECK-GRAPH-CLOSURE", subject: "recursive action and destination closure", result: blockerIds.size ? "blocked" : "pass", blocking_ids: sorted([...blockerIds].filter((id) => /^(SEARCH|WORD|KANJI|IMAGE|ACCESS)-/.test(id))), basis: blockerIds.size ? "Recorded nodes and edges are structurally complete as an index, but named runtime-access gaps prevent a coding-ready closure claim." : "Every indexed action is observed, covered by an approved app-owned contract, or explicitly excluded; no runtime-access gap blocks implementation readiness." },
  ...
  { id: "AUDIT-CHECK-FAILURE-RECOVERY", subject: "failure, recovery, persistence, offline, timing, audio, accessibility, and interaction boundaries", result: blockerIds.size ? "blocked" : "pass", blocking_ids: sorted([...blockerIds].filter((id) => /OFFLINE|AUDIO|SPEECH|TIMING|SEMANTICS|PERMISSIONS|GESTURES|REPEATABILITY|COPY-TEXT|HISTORY-DELETION/.test(id))), basis: blockerIds.size ? "Observed claims and approved boundaries are retained, but the listed gaps remain unresolved." : "Observed claims and approved platform or app-owned boundaries cover persistence, recovery, offline behavior, timing, audio, accessibility, gestures, and individual history-row deletion without inferring unknown Nihongo behavior." },
  { id: "AUDIT-CHECK-PROVENANCE", subject: "source provenance and licensing", result: [...blockerIds].some((id) => id.startsWith("PROVENANCE-")) ? "blocked" : "pass", blocking_ids: sorted([...blockerIds].filter((id) => id.startsWith("PROVENANCE-"))), basis: [...blockerIds].some((id) => id.startsWith("PROVENANCE-")) ? "Named public upstreams remain bounded by exact snapshot, transformation, provider, or licensing gaps." : "Selected pinned public sources, app-owned normalization boundaries, license constraints, and prohibited exact-reference claims are recorded without an unresolved provenance blocker." },
